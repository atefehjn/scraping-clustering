...
for i, qu in enumerate(query):
    ...
    while retry_count < max_retries:
        try:
            ...
            retry_count += 1
            print(
                f"Timeout while waiting for search results for query: {qu} (Attempt {retry_count}/{max_retries})"
            )
            ...
            if "digikala.com" in link:
                ...
                featurs_1 = waits.until(
                    EC.element_to_be_clickable(
                        (
                            By.XPATH,
                            '//*[@id="__next"]/div[1]/div[3]/div[3]/div[2]/div[2]/div[2]/div[2]/div[2]/div[1]/div[3]/button',
                        )
                    )
                )
                featurs_1.click()
                featurs_2 = waits.until(
                    EC.element_to_be_clickable(
                        (
                            By.XPATH,
                            '//*[@id="__next"]/div[1]/div[3]/div[3]/div[2]/div[5]/div[2]/div/section/span',
                        )
                    )
                )
                featurs_2.click()
                ...
                headers = waits.until(
                    EC.presence_of_all_elements_located(
                        (By.CLASS_NAME, "styles_SpecificationAttribute__value__CQ4Rz")
                    )
                )
                values = waits.until(
                    EC.presence_of_all_elements_located(
                        (
                            By.CSS_SELECTOR,
                            "div.styles_SpecificationAttribute__valuesBox__gvZeQ div.grow p.break-words:first-of-type",
                        )
                    )
                )
