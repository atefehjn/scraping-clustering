...
for i, qu in enumerate(query_te):
    print(i)
    ...
    for result in search_results:
        try:
            ...
            if 'digikala.com' in link:
                ...
                # Capture headers and their indices from the first query
                if common_headers is None:
                    common_headers = set()
                    for index, head in enumerate(headers):
                        common_headers.add(head.text)
                        header_index_map[head.text] = index  # Map header to its index

                # Create a dictionary for the model with values in the correct columns
                model_dict = {'id': model_id, 'model_code': qu}
                for head in headers:
                    th = head.text
                    if th in common_headers:  # Check if the header is common
                        index = header_index_map[th]  # Get the index from the mapping
