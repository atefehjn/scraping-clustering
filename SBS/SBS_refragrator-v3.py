    ...
    for result in search_results:
        try:
            ...
            if 'digikala.com' in link:
                ...
                # Capture headers from the first query
                if common_headers is None:
                    common_headers = set()
                    for head in headers:
                        common_headers.add(head.text)

                # Create a dictionary for the model with only common headers
                model_dict = {'id': model_id, 'model_code': qu}

                for th in common_headers:  # Iterate over common headers
                    if th in headers:  # Check if the header exists in the current headers
                        index = headers.index(th)  # Get the index of the common header
                        v = values[index].text if index < len(values) else None  # Ensure index is within bounds
                        model_dict[th] = v
                    else:
                        # Handle case where the header is not found
                        model_dict[th] = None  # Or any default value you want to use
